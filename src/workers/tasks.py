    ...
    async with db_sessionmaker() as db:
        ...
        try:
            ...
            if gen_url:
                ...
                # v1.26.2: на Railway ``app`` и ``worker`` — это два
                # отдельных контейнера без общего volume, а диск
                # эфемерный, поэтому файл, который мы только что
                # записали, виден только worker-процессу и пропадает
                # после рестарта. Чтобы картинка пережила и рестарт
                # воркера, и evict Redis-ключа (allkeys-lru),
                # сохраняем b64 прямо в ``task.result`` — это третий,
                # самый надёжный канал для `_image_available` и
                # `/storage/` fallback'ов. Ключ чистит
                # `privacy_gc_cron` через 24 ч вместе с остальными
                # артефактами. Раньше b64 писался только как
                # аварийный fallback при полном отказе Redis — то есть
                # не писался никогда в штатном сценарии, и первый же
                # деплой / evict уничтожал картинку безвозвратно.
                gen_bytes: bytes | None = None
                try:
                    gen_bytes = await storage.download(gkey)
                except Exception:
                    logger.exception(
                        "Failed to re-read generated bytes for task %s "
                        "(will try executor in-memory stash)",
                        task_id,
                    )

                if gen_bytes is None:
                    stash = analysis_result.pop("_generation_stash_jpeg", None)
                    if isinstance(stash, (bytes, bytearray)) and len(stash) > 100:
                        gen_bytes = bytes(stash)
                        logger.info(
                            "Using executor in-memory stash for task %s (%d bytes)",
                            task_id,
                            len(gen_bytes),
                        )

                analysis_result.pop("_generation_stash_jpeg", None)
                ...
                if gen_bytes is not None:
                    ...
                    if not staged:
                        logger.error(
                            "All Redis staging attempts failed for task %s — "
                            "relying on DB b64 (always-on since v1.26.2)",
                            task_id,
                        )
                    analysis_result["generated_image_b64"] = b64_gen
                else:
                    logger.critical(
                        "Task %s: generated_image_url present but bytes missing "
                        "(no storage read, no stash) — stripping public URLs",
                        task_id,
                    )
                    analysis_result.pop("generated_image_url", None)
                    analysis_result.pop("image_url", None)
