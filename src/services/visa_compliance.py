...
``data/scenarios.json`` (`prompt_overrides.analysis_checklist`). This
module exposes them to the API layer so ``/api/v1/pre-analyze`` can
...
def compliance_checklist(scenario_slug: str | None) -> list[dict[str, Any]]:
    ...
    if overrides is None or not overrides.analysis_checklist:
        return []
    return [
        {"rule": rule, "status": "pending"} for rule in overrides.analysis_checklist
    ]
