...
def load_styles_from_json() -> list[dict[str, Any]]:
    ...
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "data", "styles.json")
    try:
...
def get_structured_specs() -> list[StructuredStyleSpec]:
    ...
    for s in styles:
        ...
        spec = StructuredStyleSpec(
            ...
            allowed_variations=s.get("allowed_variations", {}).get("lighting", []) + s.get("allowed_variations", {}).get("scene", []),
            camera="",
            pose="",
            ...
            key=s["id"],
            mode=s["mode"],
            needs_full_body="yoga" in s["id"] or "running" in s["id"] or "beach" in s["id"] or "hiking" in s["id"],
