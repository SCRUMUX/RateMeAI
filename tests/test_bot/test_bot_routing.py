...
def test_bot_logs_error_if_edge_missing_in_production(
    settings_clean, monkeypatch, caplog
):
    ...
    assert any(
        "EDGE_API_URL is empty in production" in rec.message for rec in caplog.records
    )
