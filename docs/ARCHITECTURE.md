...
| **W9. Anatomy Cleanup v1.70** | `numerical_percent_anchor_enabled=false` (default), `light_match_clause_enabled=false` (default), `csl_reference_pad_enabled=true` (default) | Аудит v1.69 (`docs/ANATOMY_INVESTIGATION.md`) показал, что несмотря на padding-гейт W8, «огромная голова» всё ещё генерировалась на не-fitness стилях. Root cause — wire prompt был перегружен head-anchor клаузами: cinematic `Reframe ... bust shot ... head occupying upper third`, face-area percentage anchor, `head subtly turned`, opener tail с `head, shoulders and torso`, multi-pass `head-to-body proportions`, плюс per-framing lens spec (`85mm`/`50-70mm`/`35-50mm`) и shallow DoF — 5 head-cues vs 1 body-cue. `gym_fitness` работал как контроль потому что его `clothing.default` явно показывал плечи (`fitted athletic tank top`). v1.70 удалил **все** head-anchor клаузы из non-document пайплайна (документные стили нетронуты — vendor policy). `PHOTOREAL_BLOCK` сжат до skin texture + light match (lens/DoF удалены). Каталожная миграция v5 добавила `, shoulder line visible` в `clothing.default` для 114 не-sport, не-document, не-studio стилей. Lint-правило `NO_HEAD_TOKEN_IN_PROMPT` (через `forbidden_head_tokens_in_prompt` в `style_lint.py`) защищает от регрессий. UX: при ручном выборе `half_body`/`full_body` показывается inline-подсказка о необходимом исходнике. Wire prompt ужался с ~1450 chars до ~600–750. | `proportions_natural` pass-rate на всех framings > 95% на 7-дневном окне (раньше деградировал на portrait); жалобы «огромная голова» в support → 0 за 7 дней; subjective evaluation пользователем подтвердил естественность анатомии. |
