...
const PLATFORMS = [
  {
    ...
    getUrl: (url: string, text: string) =>
      `https://vk.com/share.php?url=${enc(url)}&title=${enc(text)}`,
    icon: (
  ...
  },
  {
    ...
    label: 'OK',
    getUrl: (url: string, text: string) =>
      `https://connect.ok.ru/offer?url=${enc(url)}&title=${enc(text)}`,
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
...
export default function ShareButtons({ url, text }: Props) {
  ...
  function handleClick(platform: typeof PLATFORMS[number]) {
    if (platform.getUrl) {
  ...
  return (
    <div className="flex flex-col gap-[var(--space-8)]">
      ...
      <div className="flex items-center justify-center gap-[var(--space-10)]">
        {PLATFORMS.map((p) => (
          <button
            key={p.id}
            onClick={() => handleClick(p)}
            title={p.id === 'instagram' ? (copied ? 'Скопировано!' : 'Скопировать ссылку') : p.label}
            className="w-10 h-10 rounded-full flex items-center justify-center text-[#E6EEF8] transition-all hover:scale-110 cursor-pointer"
