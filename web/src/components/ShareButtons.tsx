...
export default function ShareButtons({ url, text, imageUrl }: Props) {
  ...
  useEffect(() => {
    ...
    setNativeShareSupported(!!navigator.canShare?.({ files: [testFile] }));
  }, [imageUrl]);
  ...
  async function handlePlatformClick(platform: ReturnType<typeof buildPlatforms>[number]) {
    if (platform.getUrl) {
    ...
    <div className="flex flex-col gap-[var(--space-8)]">
      ...
      {nativeShareSupported && (
        <button
          ...
          disabled={sharing}
          className="mx-auto flex items-center justify-center gap-2 px-6 py-2.5 rounded-[var(--radius-12)] glass-btn-primary text-[14px] font-semibold text-white disabled:opacity-50 transition-all"
        >
          {sharing ? (
          ...
          {sharing ? 'Отправка...' : 'Отправить фото'}
        </button>
      )}
