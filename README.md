...
API и Telegram-бот: анализ фото (рейтинг, дейтинг, CV, emoji), очередь ARQ, LLM через OpenRouter, опциональная генерация изображений (Reve / Replicate / mock).
...
3. Режим **rating**: дождаться результата, проверить текст и карточку (картинка), если включена генерация.
4. Режим **dating**: ответ без обязательной картинки.
5. Режим **cv** (при необходимости указать профессию, если сценарий это спрашивает): текст + при настроенном image gen — сгенерированное фото по ссылке.
6. Режим **emoji**: текст стикеров + при настроенном image gen — изображение.
7. При ошибке пайплайна: в боте отображается сообщение из `error_message` задачи.
